...
export async function openReviewModal() {
    const weekDays = getWeekDays(currentDate);
            ...
            const { data, error } = await supabaseClient.from('weekly_reviews').select('*').eq('week_identifier', week_identifier).single();
            ...
            const saveReview = debounce(async () => {
                ...
                if (upsertError) {
                    ...
                    alert(`Lỗi khi lưu Tổng kết Tuần: ${upsertError.message}`);
                } else {
